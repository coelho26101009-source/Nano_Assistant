...
class WakeWordEngine:
    ...
    def _run(self):
        ...
        logger.warning(
            ...
            "Instala 'pvporcupine' (com PICOVOICE_ACCESS_KEY) ou 'openwakeword' e um modelo live compatível."
        )
    ...
    def _listen_openwakeword(self):
        ...
        if not self.keyword_path and self.phrase not in _openwakeword_models():
            raise _BackendUnavailable(
                f"'{self.phrase}' não tem modelo openWakeWord live disponível. "
                f"O pacote está instalado, mas o modelo de wake-word não está configurado. "
                f"Use wake_word.keyword_path para um ficheiro .onnx personalizado ou instale um modelo "
                f"compatível antes de activar a detecção."
            )
    ...
    def _load_openwakeword_model(self, model_cls):
        ...
        if self.keyword_path:
            ...
            raise _BackendUnavailable(f"openWakeWord não encontrou um modelo de wake-word para '{self.phrase}'.")
