...
if not getattr(sys, "frozen", False): load_dotenv(ROOT / ".env")
setup_logger(); logger = logging.getLogger("helios.main")
CONFIG = load_config(); guardrails = GuardrailsEngine(); memory = get_memory()
...
brain.load_history(); load_all_plugins(PLUGINS_DIR)
...
def send_message(user_text: str):
    if not user_text or not user_text.strip(): return {"error": "Mensagem vazia"}
    loop = asyncio.new_event_loop()
    try:
        asyncio.set_event_loop(loop); return loop.run_until_complete(_process_message(user_text))
    except Exception:
        logger.exception("Erro ao processar mensagem"); return {"error": "Falha ao processar o pedido", "text": "Ups Simão, ocorreu um erro ao processar o pedido."}
    finally: loop.close()
...
def get_health_status():
    """Retorna apenas estado operacional; nunca expõe API keys ou caminhos sensíveis."""
    profile = choose_model(CONFIG)
    try:
        local_ok = asyncio.run(ollama_available(brain.ollama_url.rsplit("/api/chat", 1)[0]))
        model_ok = asyncio.run(model_available(brain.ollama_model, brain.ollama_url.rsplit("/api/chat", 1)[0])) if local_ok else False
    except Exception:
    ...
    return {
        ...
        "local": {"enabled": brain.local_enabled, "model": brain.ollama_model, "available": local_ok, "modelReady": model_ok, "ramGb": profile.ram_gb},
        "memory": {"database": db_ok, "messages": memory.count_messages() if db_ok else 0},
...
def start_voice_listen():
    loop = asyncio.new_event_loop()
    try:
        asyncio.set_event_loop(loop); return {"text": loop.run_until_complete(voice.listen()) or ""}
    except Exception: logger.exception("Erro de voz"); return {"error": "Não foi possível usar o microfone."}
    finally: loop.close()
...
def get_system_stats():
    memory_info = psutil.virtual_memory(); disk = psutil.disk_usage(str(DATA_DIR))
...
def _on_wake_word():
    ...
    loop = asyncio.new_event_loop()
    try:
        asyncio.set_event_loop(loop); user_text = loop.run_until_complete(voice.listen())
        if user_text and user_text.strip():
            result = loop.run_until_complete(_process_message(user_text, blocking_tts=True)); _notify_ui(user_text, result.get("text", ""))
    except Exception: logger.exception("Erro no fluxo wake-word")
    finally:
        loop.close()
...
def _start_wake_word():
    ...
    wake_word = WakeWordEngine(voice_cfg, on_wake=_on_wake_word); wake_word.start()

async def _process_message(user_text: str, blocking_tts: bool = False) -> dict:
    try:
        memory.save_message("user", user_text); full_response = ""; status_updates = []
    ...
    except Exception: logger.exception("Erro no _process_message"); return {"text": "Ups Simão, ocorreu um erro ao processar o pedido.", "ok": False, "error": "processing_error"}

def _free_port() -> int:
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock: sock.bind(("127.0.0.1", 0)); return int(sock.getsockname()[1])

def _parse_args():
    parser = argparse.ArgumentParser(description="H.E.L.I.O.S."); parser.add_argument("--mode", default=os.getenv("HELIOS_MODE", "electron")); parser.add_argument("--port", type=int, default=0); return parser.parse_known_args()[0]

def main():
    args = _parse_args(); logger.info("HELIOS V8 a iniciar"); logger.info("Frontend: %s", FRONTEND_DIR); logger.info("Cloud: %s", "configurado" if API_KEY else "não configurado"); _start_wake_word()
    ui_cfg = CONFIG.get("ui", {}) or {}; port = args.port or int(ui_cfg.get("port", 0) or 0) or _free_port(); size = (int(ui_cfg.get("width", 1440)), int(ui_cfg.get("height", 900)))
    print(f"HELIOS_PORT={port}", flush=True)
    modes = [None] if args.mode == "electron" else [args.mode, "default"]
    for mode in modes:
        try: eel.start("index.html", mode=mode, size=size, port=port, block=True); return
