...
def describe_groq(configured_model: str = "") -> dict[str, Any]:
    ...
    if not secret["configured"]:
        return {
            ...
            "model": configured_model, "models": [], "secret": secret,
    ...
    if error:
        ...
        return {
            "id": "groq", "name": "Groq", "kind": "cloud", "role": "primary",
            "state": state.value, "model": configured_model, "models": [],
            "secret": secret, "detail": detail,
        }

    model_ok = bool(configured_model) and configured_model in models
    return {
        "id": "groq", "name": "Groq", "kind": "cloud", "role": "primary",
        "state": ProviderState.READY.value if model_ok else ProviderState.MODEL_UNAVAILABLE.value,
        "model": configured_model, "models": models, "secret": secret,
        "detail": (
            f"Pronto com '{configured_model}'." if model_ok
            else f"O modelo '{configured_model}' não existe nesta conta. Disponíveis: {', '.join(models[:4])}."
        ),
    }
...
def resolve_route(mode: ProviderMode, groq: dict, ollama: dict) -> dict[str, Any]:
    """Decide which provider a request should use, and say why.

    Never returns a provider it believes is unusable without also flagging the
    decision, so the UI can show "fell back to local" rather than pretending
    ...
    if groq_ready:
        return {
            "provider": "groq", "model": groq["model"], "usable": True,
            "fallback": False, "mode": mode.value,
