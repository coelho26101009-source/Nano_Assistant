 ...
 * Context inspector.
 *
 * Shows only what the backend reported. Any subsystem the backend has not
 * confirmed renders its real state (SETUP REQUIRED / UNAVAILABLE / UNKNOWN)
 * rather than a reassuring placeholder. Every card links to the page that owns
 * the detail, so it is a summary, never a dead end.
 */
import React from "react";
import type { CommandCenterPayload, ProviderPayload, ReadinessPayload, TaskRow } from "../lib/backend";
import type { ViewId } from "./Sidebar";
import {
  Badge, Button, EmptyState, Meter, MetricRow, Panel, Skeleton,
...
export default function Inspector({
  collapsed, onToggle, readiness, providers, commandCenter, loading,
  ...
  const toolCount = readiness ? undefined : undefined;
  ...
  return (
    <aside className="inspector" data-collapsed={collapsed} aria-label="Inspector de contexto">
      <header className="inspector__header">
        <span className="inspector__title section-label">Inspector</span>
        <Button variant="ghost" icon size="sm" onClick={onToggle} aria-label="Fechar inspector" title="Fechar (Ctrl+I)">✕</Button>
      </header>

      <div className="inspector__scroll">
        {loading && !commandCenter ? (
          <><Skeleton height={90} /><Skeleton height={120} /><Skeleton height={150} /></>
        ) : (
          <>
            {/* 1. TAREFA ATUAL */}
            <Panel
            ...
            <Panel
              ...
              action={<Button variant="ghost" size="sm" onClick={() => onNavigate("status")} aria-label="Abrir estado">↗</Button>}
            >
