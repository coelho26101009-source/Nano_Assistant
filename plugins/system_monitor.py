...
def _check_thresholds(stats: dict) -> list[str]:
    ...
    if battery is not None and not stats.get("battery_plugged") \
            and battery <= float(_cfg("battery_threshold", 15)):
...
def get_tools() -> list[dict]:
    return [
        {"type": "function", "function": {
            "name": "monitor_status",
            "description": (
                "Estado actual do PC (CPU, RAM, disco, bateria) e se algum limite "
                "está a ser ultrapassado."
            ),
