...
def add_event(title: str, start: str, end: str | None = None,
    ...
    conn = _conn()
    cur = conn.execute(
        "INSERT INTO calendar_events (title, start_at, end_at, location, notes, source) "
        "VALUES (?,?,?,?,?, 'local')",
        (title.strip(), start_dt.isoformat(timespec="minutes"),
         end_dt.isoformat(timespec="minutes") if end_dt else None, location, notes),
    )
    conn.commit()
    return {"success": True, "id": cur.lastrowid, "title": title,
            "start": start_dt.strftime("%Y-%m-%d %H:%M"),
            "message": f"Evento '{title}' marcado para {start_dt:%d/%m às %H:%M}."}
...
def list_events(days: int = 7, include_google: bool = True) -> dict:
    ...
    rows  = _conn().execute(
        "SELECT id, title, start_at, end_at, location, source FROM calendar_events "
...
def delete_event(event_id: int) -> dict:
    ...
    conn = _conn()
    cur = conn.execute("DELETE FROM calendar_events WHERE id = ? AND source = 'local'",
                       (int(event_id),))
    conn.commit()
    if cur.rowcount:
...
def import_ics(file_path: str) -> dict:
    ...
    try:
        for raw in path.read_text(encoding="utf-8", errors="ignore").splitlines():
            ...
            if line == "BEGIN:VEVENT":
                ...
                conn.execute(
                    "INSERT INTO calendar_events (title, start_at, end_at, location, source) "
                    "VALUES (?,?,?,?, 'ics')",
                    (title, start.isoformat(timespec="minutes"),
                     end.isoformat(timespec="minutes") if end else None, location or ""),
                )
                imported += 1
        conn.commit()
    ...
    return {"success": True, "imported": imported, "file": path.name,
            "message": f"Importei {imported} eventos de '{path.name}'."}
...
def _google_events(start: datetime, end: datetime) -> tuple[list[dict], str | None]:
    try:
        ...
        return [], ("Google Calendar pedido mas as bibliotecas faltam: "
                    "pip install google-api-python-client google-auth-oauthlib")

    if not GOOGLE_CREDS.exists():
        return [], f"Credenciais OAuth em falta: {GOOGLE_CREDS}"
    ...
    try:
        ...
        logger.warning(f"Google Calendar falhou: {exc}")
        return [], f"Google Calendar falhou: {exc}"
    ...
    return [
        {"type": "function", "function": {
            ...
            "parameters": {"type": "object", "required": ["title", "start"], "properties": {
                "title":    {"type": "string"},
                "start":    {"type": "string", "description": "'2026-01-02 18:30' ou '02/01/2026 18:30'"},
                "end":      {"type": "string"},
        ...
        {"type": "function", "function": {
            ...
            "description": (
                "Lista os eventos dos próximos dias (calendário local e Google, se activo). "
                "Usa quando o Simão pergunta pela agenda."
            ),
